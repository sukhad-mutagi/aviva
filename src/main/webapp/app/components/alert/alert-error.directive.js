(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('avivaaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-avivaaApp-error');
                var entityKey = httpResponse.headers('X-avivaaApp-params');
                if (errorHeader) {
